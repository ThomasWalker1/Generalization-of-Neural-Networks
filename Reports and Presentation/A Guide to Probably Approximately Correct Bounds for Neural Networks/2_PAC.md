...
**Theorem 2.5.1 (Hoeffding's Inequality)** Let $X_1,\dots,X_n$ be independent random variables with range $[a,b]$. Let $X=\sum_{i=1}^nX_i$ and $\mu=\mathbb{E}(X)$, then for $t>0$ we have that $$(i)\quad\mathbb{P}\left(X\leq\mu-t\right)<\exp\left(-\frac{2t^2}{n(b-a)^2}\right)\text{ and }(ii)\quad\mathbb{P}\left(X\geq\mu+t\right)<\exp\left(-\frac{2t^2}{n(b-a)^2}\right).$$
<details>
<summary>Proof</summary>
<br>
